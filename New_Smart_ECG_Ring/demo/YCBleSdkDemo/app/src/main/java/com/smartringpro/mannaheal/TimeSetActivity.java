...
public class TimeSetActivity extends Activity {
    ...
    private void initView() {
        ...
        findViewById(R.id.send_value_to_device).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                /****
                 * 下发测量数据
                 * @param type    类型  0x00:心率  0x01:血压  0x02:血氧  0x03:呼吸率  0x04:HRV  0x05:血糖  0x06:温度   (暂时只有血糖生效，其它的是为了以后扩展)
                 * @param time    时间戳
                 * @param value1  类型的值1
                 * @param value2  类型的值2   默认为0   血压:舒张压DBP  温度:小数部分  血糖:小数部分
                 * @param dataResponse code 0x00:下发指令成功   0x01:下发指令失败
                 *                     byte[] data = hashMap.get("datas")  data[0]表示型号和type对应  data[1]  0表示下发成功  1表示下发失败
                 */
                YCBTClient.appSendMeasureNumber(5, 1670938122, 12, 5, new BleDataResponse() {
                    @Override
                    public void onDataResponse(int code, float ratio, HashMap resultMap) {
                        if (code == 0 && resultMap != null) {
                            byte[] datas = (byte[]) resultMap.get("datas");
                            if (datas != null && datas.length == 2 && datas[0] == 5) {
                                if (datas[1] == 0)
                                    System.out.println("chong----------setMeasureNumber---success");
                                else
                                    System.out.println("chong----------setMeasureNumber---failed");
                            }
                        }
                    }
                });
        ...
        findViewById(R.id.start_blood_sugar).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        YCBTClient.appRegisterRealDataCallBack(new BleRealDataResponse() {
            ...
            public void onRealDataResponse(int dataType, HashMap dataMap) {
                System.out.println("chong------dataMap==" + dataMap.get("data").toString());
